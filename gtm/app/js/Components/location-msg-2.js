...
$(document).ready(function () {
  ...
  $(".info").click(function (event) {
    ...
    if (a.length === 1 && b.length === 0) {
      // If the source of the image contains the string "close.svg", change it to "info.svg"
      if (el.src.indexOf("images/header/close.svg") > 0) {
        el.src = "../images/header/info.svg";
      } else {
        // Otherwise, set the source to "info.svg"
        el = "../images/header/info.svg";
      }
    } else {
      // If there is more than one element with class "info" or at least one element with classes "info" and "active"

      // If the source of the image contains the string "info.svg", change it to "close.svg"
      if (el.src.indexOf("images/header/info.svg") > 0) {
        el.src = "../images/header/close.svg";
      } else {
        // Otherwise, set the source to "info.svg"
        el = "../images/header/info.svg";
      }
...
setTimeout(function (element) {
  ...
  if (a.length === 1 && b.length === 0) {
    // If the 'info-img' source contains 'images/close.svg', changes it to 'images/info.svg'.
    if (el.src.indexOf("images/header/close.svg") > 0) {
      el.src = "../images/header/info.svg";
      // Otherwise, sets the 'info-img' source to 'images/info.svg'.
    } else {
      el = "images/info.svg";
    }
    // If there is more than one 'info' element or at least one 'info active' element.
  } else {
    // If the 'info-img' source contains 'images/info.svg', changes it to 'images/close.svg'.
    if (el.src.indexOf("images/header/info.svg") > 0) {
      el.src = "../images/header/close.svg";
      // Otherwise, sets the 'info-img' source to 'images/info.svg'.
    } else {
